...
const RRouterProviders=[
  ...
  {value:"baseten",label:"Baseten",description:"Route through your Baseten account and selected hosted model.",kind:"key",secret:"BASETEN_API_KEY"}
...
function RRouterCredential({provider:s,state:e,keys:t,onSaved:n}){const[r,i]=de.useState(""),[o,l]=de.useState(!1),[err,setErr]=de.useState(null);if(s.kind==="account")return a.jsx(se,{as:"span",color:"secondary",size:"sm",children:"Signed in"});if(s.kind==="local"){const c=e.local?.[s.localKey],d=c?.installed&&c?.authenticated;return a.jsx(se,{as:"span",color:d?"primary":"secondary",size:"sm",children:d?"Ready":c?.installed?"Sign in with "+(s.value==="codex"?"codex login":"claude"):"Not installed"})}const c=t.includes(s.secret),d=async()=>{if(r.trim().length===0)return;l(!0);setErr(null);try{const u=await window.desktop.secrets.upsert({[s.secret]:r.trim()});i("");n();if(u&&u.synced===false)setErr("Key saved. Computer sync is catching up — try chat again in a moment.")}catch(u){setErr(String(u?.message??u))}finally{l(!1)}};return a.jsxs("div",{className:"sand-9f619 sand-78zum5 sand-dt5ytf sand-h8yej3",style:{width:360,gap:8},children:[a.jsxs("div",{className:"sand-9f619 sand-78zum5 sand-6s0dn4 sand-h8yej3",children:[a.jsx("input",{"aria-label":s.secret,className:RRouterInputClass,disabled:o,onChange:u=>i(u.currentTarget.value),placeholder:c?"Replace saved key":"Paste API key",style:{fontSize:13,height:34,minWidth:0,padding:"0 10px",width:270},type:"password",value:r}),a.jsx(oe,{disabled:o||r.trim().length===0,onClick:d,shape:"rectangular",size:"sm",variant:"secondary",children:o?"Saving…":"Save"})]}),err?a.jsx(se,{as:"p",color:"red",size:"sm",children:err}):null]})}
