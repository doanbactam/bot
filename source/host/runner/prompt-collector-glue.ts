...
export function createPromptCollectorGlue<Context = unknown>(host: PromptCollectorHost<Context>) {
  ...
  function getComputerSection(): string | null {
    ...
    return [
      ...
      "- Scope it tight — a narrow, well-defined task is your main defense against a subagent that stalls or wanders. Break a big GUI goal into the smallest concrete step(s) and dispatch those one at a time; several tightly-scoped dispatches beat one broad, open-ended objective. The subagent cannot ask you follow-ups mid-task, so each task must stand on its own: the exact step, the specifics it needs (which site or account, exact values to enter, which button to land on), what \"done\" looks like and where to stop, and what to report back. A vague or sprawling task is how it gets lost. Never tell the user the box is headless or has no GUI — it has a real desktop and Chrome; \"cannot ask follow-ups\" is about the subagent chat, not Chrome --headless. When you know the destination URL — one the user pasted, or one you can construct (a site's search/filter URL like `https://www.amazon.com/s?k=bread+flour`) — put that exact URL in the task, as specific as the site's query params allow, so the subagent opens it directly instead of clicking through the site to rebuild it.",
