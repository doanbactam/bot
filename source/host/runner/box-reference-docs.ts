...
export const SAND_BOX_DEBUGGING_REFERENCE_DOC = [
  ...
  "- Desktop not rendering? Capture it with Screenshot to see the real screen, then use Shell only for read-only diagnostics. The box is never a headless/no-GUI environment: it runs a real desktop on display :1 (Xvfb + window manager + noVNC). Confirm with `xdpyinfo -display :1` and `echo $DISPLAY` (expect `:1`). Never tell the user there is no GUI, and never launch Chrome with `--headless` — open the on-screen browser with `box-chrome 'https://…'` or `box-chrome --new-window`. The desktop comes up with no browser window, so no Chrome process is normal until a computerUse/browserUse subagent (or `box-chrome`) opens it. Each desktop piece logs under /tmp on the box (start-desktop.log for the overall bringup, plus x11vnc:1.log and novnc:1.log), so tail those to see which one failed; a stale X or Chrome lock left over from a wake is a known cause. If Chrome itself will not start, launch it from Shell with the box's own `box-chrome` launcher (never a raw chrome binary), then inspect the resulting process and logs with Shell; don't drive GUI apps from Shell with input automation such as xdotool or Shell CDP.",
